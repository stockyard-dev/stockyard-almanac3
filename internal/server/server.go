package server
import("encoding/json";"net/http";"github.com/stockyard-dev/stockyard-almanac3/internal/store")
...
func(s *Server)routes(){
    s.mux.HandleFunc("GET /health",func(w http.ResponseWriter,r *http.Request){writeJSON(w,200,map[string]string{"status":"ok","service":"stockyard-almanac3"})})
    s.mux.HandleFunc("GET /api/stats",s.handleOverview)
    s.mux.HandleFunc("GET /api/tables",s.handleListTables)
    s.mux.HandleFunc("POST /api/tables",s.handleCreateTable)
    s.mux.HandleFunc("DELETE /api/tables/{id}",s.handleDelete)
    s.mux.HandleFunc("GET /api/tables/{id}/columns",s.handleListColumns)
    s.mux.HandleFunc("POST /api/tables/{id}/columns",s.handleAddColumn)
