...
if(i.database)h+='<div class="item-sub">'+esc(i.database)+'</div>';
...
if(i.table_name)h+='<span>'+esc(i.table_name)+'</span>';
if(i.columns)h+='<span>'+esc(i.columns)+'</span>';
if(i.owner)h+='<span>'+esc(i.owner)+'</span>';
...
async function del(id){if(!confirm('Delete?'))return;await fetch(A+'/schemas/'+id,{method:'DELETE'});load();}
function formHTML(item){var i=item||{name:"",database:"",table_name:"",columns:"",description:"",owner:"",version:""};var isEdit=!!item;
...
h+='<div class="fr"><label>Table Name</label><input id="f-table-name" value="'+esc(i.table_name)+'"></div>';
h+='<div class="fr"><label>Columns</label><input id="f-columns" value="'+esc(i.columns)+'"></div>';
h+='<div class="fr"><label>Description</label><textarea id="f-description" rows="3">'+esc(i.description)+'</textarea></div>';
h+='<div class="fr"><label>Owner</label><input id="f-owner" value="'+esc(i.owner)+'"></div>';
...
async function submit(){var body={name:document.getElementById('f-name').value.trim(),database:document.getElementById('f-database').value.trim(),table_name:document.getElementById('f-table-name').value.trim(),columns:document.getElementById('f-columns').value.trim(),description:document.getElementById('f-description').value.trim(),owner:document.getElementById('f-owner').value.trim(),version:document.getElementById('f-version').value.trim()};
if(editId){await fetch(A+'/schemas/'+editId,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}
else{await fetch(A+'/schemas',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}closeModal();load();}
